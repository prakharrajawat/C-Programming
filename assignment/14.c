/*
Question 14. We would like you to develop a program capable of making a child play automatically the game of "more or less" -- the child must try to guess a secret number. The program should respond to each guess with "it is more" or "it is less" until the child finds the right number.
...
#include <stdio.h>
int guess_the_num(int secret_num);

int main()
{
  int secret_num;
  printf("enter no. to be guessed :");
  scanf("%d", &secret_num);

  printf("Number of tries needed :%d", guess_the_num(secret_num));
  return 0;
}
...
int guess_the_num(int secret_num)
{
  ...
  for (tries = 0; guess != secret_num; tries++)
  {
    scanf("%d", &guess);
    if (guess < secret_num)
    {
